...
namespace GetVideoHandler
{
    /// <summary>
    /// Summary description for GetVideo
    /// </summary>
    public class GetVideo : IHttpHandler, IReadOnlySessionState
    {
        public GetVideo()
        {
            //
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!String.IsNullOrEmpty(sId))
            {
                ...
                if (con != null)
                {
                    ...
                    {
                        con.Open();
                        MySqlCommand cmd = new MySqlCommand("select BandId, Visibility, Author, FileName from video where Deleted=0 and Id=?videoId", con);
                        cmd.Parameters.Add("?videoId", MySqlDbType.UInt64).Value = UInt64.Parse(sId);
